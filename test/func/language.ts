...
describe('updateLanguageSet', () => {
	...
	beforeEach(function () {
		return Promise.all([
			new Language(firstLanguageAttribs).save(null, {
				method: 'insert'
			}),
			new Language(secondLanguageAttribs).save(null, {
				method: 'insert'
			})
		]);
	});
	afterEach(function () {
		return truncateTables(bookshelf, ['musicbrainz.language']);
	});

	/* eslint-disable-next-line max-len */
	it('should return null if all null/empty sets are passed', async function () {
		const result = await bookshelf.transaction((trx) =>
			updateLanguageSet(bookbrainzData, trx, null, []));
	...
	it('should return a set with one language if one language is added to an empty set', async function () {
		const result = await bookshelf.transaction(async (trx) => {
			const set = await updateLanguageSet(bookbrainzData, trx, null, [
				{
					id: 1
				}
			]);
	...
	it('should return the old set if no changes are made', async function () {
		...
		const result = await bookshelf.transaction(async (trx) => {
			const set = await updateLanguageSet(bookbrainzData, trx, firstSet, [
				{
					id: 1
				},
				{
