...
describe('updateIdentifierSet', () => {
	...
	beforeEach(function () {
		return new IdentifierType(idTypeAttribs).save(null, {
			method: 'insert'
		});
	});
	afterEach(function () {
	...
	it('should return null if all null/empty sets are passed', async function () {
		const result = await bookshelf.transaction((trx) =>
			updateIdentifierSet(bookbrainzData, trx, null, []));
	...
	it('should return a set with one identifier if one identifier is added to an empty set', async function () {
		...
		const result = await bookshelf.transaction(async (trx) => {
			const set = await updateIdentifierSet(bookbrainzData, trx, null, [
				identifierData
			]);
			return set.refresh({
				transacting: trx,
	...
	it('should return a new set if changes are made to the identifiers in the set', async function () {
		...
		const firstSetIdentifiers = _.orderBy(
			firstSet.related('identifiers').toJSON(),
			'id'
		);

		const thirdIdentifierData: any = getIdentifierData();
		...
		const identifiers = _.orderBy(
			result.related('identifiers').toJSON(),
			'id'
		);
