...
describe('createNewSetWithItems', () => {
	...
	beforeEach(function () {
		return new Language(languageAttribs).save(null, {
			method: 'insert'
		});
	});
	afterEach(function () {
	...
	it('should error out if non-existent unchanged items are provided', function () {
		const resultPromise = bookshelf.transaction((trx) =>
			createNewSetWithItems(
				bookbrainzData,
				trx,
				AliasSet,
	...
	it('should return a new set with one item if an item is added', async function () {
		...
		const resultSet = await bookshelf.transaction(async (trx) => {
		...
		expect(items[0]).to.include({...aliasData, languageId: null});
	});
	...
	it('should return a new set with two items if an item is added to an existing unchanged item', async function () {
		...
		const firstSetItems = _.orderBy(
			firstSet.related('aliases').toJSON(),
			'id'
		);
		...
		expect(items[0]).to.include({...firstAliasData, languageId: null});
		expect(items[1]).to.include({...secondAliasData, languageId: null});
	});
});
