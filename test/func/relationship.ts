...
async function makeEntity(bbid) {
	...
	await new Revision(revisionAttribs).save(null, {
		method: 'insert'
	});
	...
	const entity = await new Work(workAttribs).save(null, {
		method: 'insert'
	});
...
describe('updateRelationshipSet', () => {
	before(async function () {
		await Promise.all([
			new Gender(genderData).save(null, {
				method: 'insert'
			}),
		...
		await new Editor(editorAttribs).save(null, {
			method: 'insert'
		});
		return Promise.all([
			new RelationshipType(getRelationshipTypeData(1)).save(null, {
				method: 'insert'
			}),
	...
	it('should return an empty object if all null/empty sets are passed', async function () {
		const result = await bookshelf.transaction((trx) =>
			updateRelationshipSets(bookbrainzData, trx, null, []));
	...
	it('should return a new set if changes are made to the relationships in the set', async function () {
		...
		const firstResult = await bookshelf.transaction(async (trx) => {
			...
			const refreshedSetsPromise = BBPromise.props(
				_.transform(
					sets,
					(acc, set, bbid) => {
						acc[bbid] = set.refresh({
			...
			return BBPromise.join(
				refreshedSetsPromise,
				updatedEntitiesPromise,
				(refreshedSets) => refreshedSets
			);
		});
		const firstSet = firstResult[aBBID];

		const firstSetRelationships = _.orderBy(
			firstSet.related('relationships').toJSON(),
			'id'
		).map((relationship) =>
		...
		const aBBIDRelationships = _.orderBy(
			result[aBBID].related('relationships').toJSON(),
			'id'
		);

		const cBBIDRelationships = result[cBBID];

		const dBBIDRelationships = _.orderBy(
			result[dBBID].related('relationships').toJSON(),
			'id'
		);

		expect(result)
			.to.be.an('object')
			.that.has.all.keys(aBBID, cBBID, dBBID);
		expect(aBBIDRelationships[0])
			.to.be.an('object')
			.to.include(firstRelationshipData);
		expect(aBBIDRelationships[1])
			.to.be.an('object')
			.to.include(thirdRelationshipData);
	...
	it('should return the old set if no changes are made', async function () {
		...
		const firstSet = firstResult[aBBID];
		const firstSetRelationships = firstSet
