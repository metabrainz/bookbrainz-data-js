...
describe('recursivelyGetRedirectBBID', () => {
	...
	it('should return a single-step redirected bbid', async function () {
		const redirectedBBID = await recursivelyGetRedirectBBID(
			bookbrainzData,
			aBBID
		);
...
describe('getEntity', () => {
	async function createAuthorWithData(index, bbid) {
		...
		await new Disambiguation({
			comment: `Test Disambiguation ${index}`,
			id: index
		}).save(null, {
			method: 'insert'
		});
		...
		await new Author(authorAttribs).save(null, {
			method: 'insert'
		});
	}
	...
	it('should return an entity with extra relations', async function () {
		...
		const entityJSON = await getEntity(
			bookbrainzData,
			'Author',
			aBBID,
			relations
	...
	it('should return the target entity for a redirected bbid', async function () {
		const redirectedEntityJSON = await getEntity(
			bookbrainzData,
			'Author',
			bBBID
		);
