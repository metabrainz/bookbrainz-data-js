...
const {expect} = chai;
const {
	AliasSet, Annotation, Disambiguation, EditionGroup, EditionGroupRevision, Editor,
...
const data = {
	...
	editionGroup: {
		aliasSetId: 1,
		annotationId: 1,
		bbid: '9b1b9cf4-aeda-11eb-8529-0242ac130003',
		disambiguationId: 1,
		identifierSetId: 1,
		relationshipSetId: 1,
		revisionId: 1
...
describe('EditionGroupRevision model', () => {
	beforeEach(
		async () => {
			...
			await new Annotation(data.annotation)
				.save(null, {method: 'insert'});
			await new EditionGroup(data.editionGroup)
				.save(null, {method: 'insert'});
		}
	);
	...
	it('should return a JSON object with correct keys when saved', async () => {
		...
		const revision = await new EditionGroupRevision({id: 1}).fetch({withRelated: relatedToLoad});
