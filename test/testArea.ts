...
describe('AreaType model', () => {
	afterEach(() =>
		truncateTables(bookshelf, ['musicbrainz.area', 'musicbrainz.area_type']));
	it('should return a JSON object with correct keys when saved', async () => {
		const areaType = await new AreaType({
			id: 1,
			name: 'Planet'
		})
			.save(null, {
	...
	it('should be fetched as relationship when fetching Area', async () => {
		...
		const fetchedArea = await Area.forge({
			id: createdArea.id
		}).fetch({
			withRelated: 'areaType'
		});
		expect(fetchedArea.related('areaType')?.toJSON()).to.deep.equal(
			areaType
		);
	});
});
