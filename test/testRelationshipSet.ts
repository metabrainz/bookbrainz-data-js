...
describe('RelationshipSet model', () => {
	beforeEach(() =>
		new RelationshipType(relTypeAttribs)
			...
			.then(() =>
				new Entity(entityAttribs).save(null, {
					method: 'insert'
				}))
			.then(() =>
				new Entity(
	...
	it('should return a JSON object with correct keys when saved', () => {
		const jsonPromise = new RelationshipSet({
			id: 1
		})
			.save(null, {
				method: 'insert'
	...
	it('should have an empty list of relationships when none are attached', () => {
		const jsonPromise = new RelationshipSet({
			id: 1
		})
			.save(null, {
	...
	it('should have have a relationship when one is set', () => {
		const relPromise = new Relationship(relAttribs).save(null, {
			method: 'insert'
		});
		const jsonPromise = relPromise
			.then((relationship) => createRelationshipSet([relationship]))
			.then((model) =>
				model.refresh({
					withRelated: ['relationships']
				}))
	...
	it('should have have two relationships when two are set', () => {
		const rel1Promise = new Relationship(relAttribs).save(null, {
			method: 'insert'
		});
		const rel2Promise = new Relationship(
			_.assign(relAttribs, {
				id: 2
			})
		).save(null, {
