...
const AchievementRank = require('../index').AchievementRank;

describe('AchievementRank model', () => {
	...
	afterEach(function truncate() {
		...
		return util.truncateTables(Bookshelf, [
			...
			'bookbrainz.editor', 'bookbrainz.achievement_rank',
			'bookbrainz.achievement_type'
		]);
	});

	it('should return a JSON object with correct keys when saved', () => {
		const unlockPromise = new AchievementRank({id: 1, editorId: 1,
			achievementId: 1, achievementRank: 2})
			.save(null, {method: 'insert'})
			.then((model) => model.refresh())
			.then((unlock) => unlock.toJSON());

		return expect(unlockPromise).to.eventually.have.all.keys([
			'id', 'editorId', 'achievementId', 'achievementRank'
		]);
	});
});
