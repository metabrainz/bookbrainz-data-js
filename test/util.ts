...
const INVALID_DATES_TO_FORMAT = [
	...
	['a'],
	['a', 1],
	[null, 1, 3], // eslint-disable-next-line no-undefined
	[undefined, 1, 3]
];
...
describe('Utils', () => {
	describe('parseDate', () => {
		it('should return an array of 3 null values when passed an invalid string (other than ISO 8601)', () => {
			const result = INVALID_DATES_TO_PARSE.reduce(
				(res, date) => res || !isDateInvalid(date),
				false
			);
	...
	describe('formatDate', () => {
		it('should return null when passed an invalid date object', () => {
			const result = INVALID_DATES_TO_FORMAT.reduce(
				(res, date) => res || formatDate(...date) !== null,
				false
			);
			expect(result).to.be.false;
		});
