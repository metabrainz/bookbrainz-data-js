...
describe('AliasSet model', () => {
	...
	beforeEach(() =>
		new Language(languageAttribs).save(null, {
			method: 'insert'
		}));
	...
	it('should return a JSON object with correct keys when saved', () => {
		const jsonPromise = new AliasSet({
			id: 1
		})
			.save(null, {
				method: 'insert'
			...
			.then((model) =>
				model.refresh({
					withRelated: ['aliases', 'defaultAlias']
				}))
	...
	it('should have have an alias when one is set', () => {
		const aliasPromise = new Alias(aliasAttribs).save(null, {
			method: 'insert'
		});
	...
	it('should have have two aliases when two are set', () => {
		const alias1Promise = new Alias(aliasAttribs).save(null, {
			method: 'insert'
		});
		const alias2Promise = new Alias(
			_.assign(aliasAttribs, {
				id: 2
			})
		).save(null, {
			method: 'insert'
