...
function createEdition() {
	return bookshelf.transaction(async function (transacting) {
		...
		const edition = await new Edition(editionAttribs).save(null, {
			method: 'insert',
			transacting
		});
		return edition;
...
describe('Edition model', () => {
		...
		new Gender(genderData)
			...
			.then(() =>
				Promise.all([
					...
					new AuthorCredit({...setData, authorCount: 0}).save(null, {
						method: 'insert'
					}),
					new Disambiguation({
	...
	it('should automatically create an Edition Group editionGroupBbid has been unset in existing Edition', async () => {
		...
		const revision2 = await new Revision({
			...revisionAttribs,
			id: 2
		}).save(null, {
			method: 'insert'
		});
		edition = await new Edition({
			...editionAttribs,
			editionGroupBbid: null,
			revisionId: revision2.id
		}).save();
		// Fetch it again
		edition = await new Edition({
			bbid: edition.get('bbid')
		}).fetch({
			withRelated: ['editionGroup']
		});
	...
	it('should return the master revision when multiple revisions exist', () => {
		...
		const revisionOnePromise = new Revision(revisionAttribs2).save(null, {
			method: 'insert'
		});
		const editionPromise = revisionOnePromise
			.then(() =>
				new Edition(editionAttribs2).save(null, {
					method: 'insert'
				}))
			.then((model) => model.refresh())
			.then((author) => author.toJSON());
		const revisionTwoPromise = editionPromise.then(() => {
			revisionAttribs2.id = 2;
			return new Revision(revisionAttribs2).save(null, {
				method: 'insert'
			});
		});
		const editionUpdatePromise = BBPromise.join(
			editionPromise,
			revisionTwoPromise,
			(edition) => {
		...
		return Promise.all([
			expect(editionUpdatePromise).to.eventually.have.property(
				'revisionId',
				2
			),
			expect(editionUpdatePromise).to.eventually.have.property(
