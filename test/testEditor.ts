...
const {expect} = chai;
const {Editor, EditorType, Gender, Revision, bookshelf} = bookbrainzData;
const genderAttribs = {
	id: 1,
	name: 'test'
};
...
describe('Editor model', () => {
	beforeEach(() =>
		new Gender(genderAttribs)
			.save(null, {
				method: 'insert'
			})
			.then(() =>
				new EditorType(editorTypeAttribs).save(null, {
					method: 'insert'
				})));
		...
		const editorPromise = new Editor(editorAttribsWithOptional)
			...
			.then(() =>
				new Editor({
					...
					withRelated: ['type', 'gender', 'revisions']
				}))
