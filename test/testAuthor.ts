...
describe('Author model', () => {
	...
	it('should return a JSON object with correct keys when saved', () => {
		...
		const authorPromise = annotationPromise
			.then(() =>
				new Author(authorAttribs).save(null, {
					method: 'insert'
				}))
			.then((model) =>
				model.refresh({
	...
	it('should return the master revision when multiple revisions exist', () => {
		...
		const authorPromise = revisionOnePromise
			...
			.then((author) => author.toJSON());
		const revisionTwoPromise = authorPromise.then(() => {
			revisionAttribs.id = 2;
			return new Revision(revisionAttribs).save(null, {
				method: 'insert'
			});
		});
		const authorUpdatePromise = BBPromise.join(
			authorPromise,
			revisionTwoPromise,
			(author) => {
			...
			.then((author) => author.toJSON());
		return Promise.all([
			expect(authorUpdatePromise).to.eventually.have.property(
				'revisionId',
				2
			),
			expect(authorUpdatePromise).to.eventually.have.property(
