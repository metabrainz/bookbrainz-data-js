...
describe('Message model', function() {
  ...
  it('should return a JSON object with correct keys when saved', function() {
    var messagePromise = new Message({senderId: 1, subject: 'test', content: 'test'}).save()
    .then(function(model) {
      return model.refresh({withRelated: ['sender']}).then(util.fetchJSON);
    });
  ...
  it('should include a filled out sender relation if requested', function() {
    var messagePromise = new Message({senderId: 1, subject: 'test', content: 'test'}).save()
    .then(function(model) {
      return model.refresh({withRelated: ['sender']}).then(function(model){
