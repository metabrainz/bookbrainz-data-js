...
describe('EditionRevision model', () => {
	beforeEach(() =>
		new Gender(data.gender)
			...
			.then(() =>
				new EditorType(data.editorType).save(null, {
					method: 'insert'
				}))
			.then(() =>
				new Editor(data.editor).save(null, {
					method: 'insert'
				}))
			.then(() =>
				Promise.all([
					new AliasSet(data.set).save(null, {
						method: 'insert'
					}),
					new IdentifierSet(data.set).save(null, {
						method: 'insert'
					}),
					new RelationshipSet(data.set).save(null, {
						method: 'insert'
					}),
					new Disambiguation(data.disambiguation).save(null, {
						method: 'insert'
					}),
					new Entity({
						bbid: 'de305d54-75b4-431b-adb2-eb6b9e546014',
			...
			.then(() =>
				new Revision(data.revision).save(null, {
					method: 'insert'
				}))
			.then(() =>
				new Annotation(data.annotation).save(null, {
					method: 'insert'
				}))
			.then(() =>
				new Edition(data.edition).save(null, {
					method: 'insert'
				})));
	...
	it('should return a JSON object with correct keys when saved', () => {
		...
		const revisionPromise = new EditionRevision({
			id: 1
		})
			.fetch({
				withRelated: relatedToLoad
			})
