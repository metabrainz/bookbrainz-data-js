...
const {expect} = chai;
const {Identifier, IdentifierSet, IdentifierType, bookshelf} = bookbrainzData;
const idAttribs = {
	id: 1,
	typeId: 1,
	value: 'Bob'
...
describe('IdentifierSet model', () => {
	...
	beforeEach(() =>
		new IdentifierType(idTypeAttribs).save(null, {
			method: 'insert'
		}));
	afterEach(() =>
	...
	it('should return a JSON object with correct keys when saved', () => {
		const jsonPromise = new IdentifierSet({
			id: 1
		})
			.save(null, {
				method: 'insert'
	...
	it('should have an empty list of identifiers when none are attached', () => {
		const jsonPromise = new IdentifierSet({
			id: 1
		})
			.save(null, {
	...
	it('should have have an identifier when one is set', () => {
		const idPromise = new Identifier(idAttribs).save(null, {
			method: 'insert'
		});
		const jsonPromise = idPromise
			.then((identifier) => createIdentifierSet([identifier]))
			.then((model) =>
				model.refresh({
					withRelated: ['identifiers']
				}))
		...
		return BBPromise.all([
			expect(jsonPromise).to.eventually.have.nested.property(
				'identifiers[0].id',
				1
			)
		]);
	...
	it('should have have two identifiers when two are set', () => {
		const id1Promise = new Identifier(idAttribs).save(null, {
			method: 'insert'
		});
		const id2Promise = new Identifier(
			_.assign(idAttribs, {
				id: 2
			})
		).save(null, {
		...
		const jsonPromise = BBPromise.join(
			id1Promise,
			id2Promise,
			...
			.then((model) => model.toJSON());
		return expect(jsonPromise).to.eventually.have.nested.property(
			'identifiers.length',
			2
		);
	});
