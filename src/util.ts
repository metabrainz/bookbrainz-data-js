	...
	function sortEntityData(data) {
		...
		if (identifiersPresent) {
			data.identifierSet.identifiers = _.sortBy(
				data.identifierSet.identifiers,
				['value', 'type.label']
			);
		}

		const relationshipsPresent =
			data.relationshipSet &&
			_.isArray(data.relationshipSet.relationships);

		if (relationshipsPresent) {
			data.relationshipSet.relationships = _.sortBy(
				data.relationshipSet.relationships,
				'id'
			);
		}
 ...
 *                   date.
 */
export function formatDate(year = null, month = null, day = null) {
	...
	const yearString = `${isCommonEraDate ? '+' : '-'}${_.padStart(
 ...
 * The Edition Group will be part of the same revision, and will have the same alias set and author credit
 * for that revision
