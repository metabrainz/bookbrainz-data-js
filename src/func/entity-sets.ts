...
function updateEntitySet(
	...
	const oldItems = oldSet ? oldSet.related(derivedSet.propName).toJSON() : [];
	const comparisonFunc = getComparisonFunc([
		derivedSet.idField,
		...derivedSet.mutableFields || []
	]);
	...
	const removedItems = getRemovedItems(oldItems, newItems, comparisonFunc);
...
export async function updateEntitySets(
	...
	const newProps = await Promise.all(
		derivedSets.map(async (derivedSet) => {
			...
			const oldSetRecord = await derivedSet.model
				.forge({
			...
			return {
				[derivedSet.entityIdField]: newSetRecord ?
					newSetRecord.get('id') :
					null
			};
		})
