...
function getAffectedBBIDs(
	addedItems: Array<Relationship>,
	removedItems: Array<Relationship>
): Array<string> {
...
async function getMasterRelationshipSetForEntity(
	...
	const entity = await typeModelMap[type]
		.forge({
			bbid
		})
		.fetch({
...
async function updateRelationshipSetForEntity(
	...
	const oldSet = await getMasterRelationshipSetForEntity(
		orm,
		transacting,
		bbid
	);
	const oldSetItems: Array<Relationship> = oldSet ?
		oldSet.related('relationships').toJSON() :
		[];
	const addedItems = allAddedItems.filter(
		(relationship) =>
			relationship.sourceBbid === bbid || relationship.targetBbid === bbid
	);
	const unchangedItems = removeItemsFromSet(
		oldSetItems,
		allRemovedItems,
		comparisonFunc
	);
...
export function updateRelationshipSets(
	...
	const allAddedItems = getAddedItems(
		oldSetItems,
		newSetItems,
		comparisonFunc
	);
	const allRemovedItems = getRemovedItems(
		oldSetItems,
		newSetItems,
		comparisonFunc
	);
	...
	return BBPromise.props(newSetPromises);
}
