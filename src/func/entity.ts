...
export function getAdditionalEntityProps(
	...
	if (
		entityType === entityTypes.EDITION_GROUP ||
 ...
 */
export function getEntitySetMetadataByType(
	entityType: string
): Array<any> {
 ...
 */
export function getEntityModels(orm: any): any {
 ...
 * single param
 */
export function getEntityModelByType(
	orm: any,
	type: string
): any {
 ...
 */
export async function recursivelyGetRedirectBBID(
	orm: any,
	bbid: string,
	transacting?
) {
	const redirectSQLQuery = `SELECT target_bbid FROM bookbrainz.entity_redirect WHERE source_bbid = '${bbid}'`;
	const redirectQueryResults = await (transacting || orm.bookshelf.knex).raw(
		redirectSQLQuery
	);
...
export async function getEntityParentAlias(orm, entityType, bbid) {
	...
	const rows = queryResult.rows.map((rawQueryResult) => {
		...
		const queriedResult = _.mapKeys(rawQueryResult, (value, key) =>
			_.camelCase(key));
