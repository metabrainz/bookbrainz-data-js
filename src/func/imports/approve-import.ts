...
import {getAdditionalEntityProps, getEntityModelByType, getEntitySetMetadataByType} from '../entity';
...
export async function approveImport({
	...
	const editorUpdatePromise = incrementEditorEditCountById(
		orm,
		editorId,
		transacting
	);
	...
	const notePromise = revisionPromise.then((revision) =>
		createNote(orm, note, editorId, revision, transacting));
	...
	const entitySetMetadata: Array<any> = getEntitySetMetadataByType(entityType);
	const entitySets = entitySetMetadata.reduce(
		(set, {entityIdField}) =>
			_.assign(set, {
				[entityIdField]: importEntity[entityIdField]
			}),
		{}
	);
	...
	const entityModel = await model.forge(propsToSet).save(null, {
		method: 'insert',
		transacting
	});
