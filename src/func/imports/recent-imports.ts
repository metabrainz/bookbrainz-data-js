...
async function getRecentImportUtilData(
	...
	const recentImportUtilsData: any = await transacting
		.select(
			'link.imported_at',
		...
		.join(
			transacting
				.select('import_id', 'imported_at', 'origin_source_id')
	...
	/* Returns the holder object which has two fields:
  	...
  	*/
	return recentImportUtilsData.reduce(
		(holder: any, data: any) => {
...
export async function getRecentImports(
  ...
  */
	...
	const defaultAliasesMap = await getAliasByIds(transacting, defaultAliasIds);
	...
	return recentImports.map((recentImport) => {
		// Add timestamp
		recentImport.importedAt = moment(
			timeStampMap[recentImport.import_id]
		).format('YYYY-MM-DD');
		// Add origin source
...
export async function getTotalImports(transacting: Transaction) {
	const [{count}] = await transacting('bookbrainz.link_import').count(
		'import_id'
	);
