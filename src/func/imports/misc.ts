...
export async function originSourceMapping(
	...
	const mappingRecord = await transacting
		.select('*')
		.from('bookbrainz.origin_source');
	return mappingRecord.reduce((mapping, {id, name}) => {
		if (idAsKey) {
			return _.assign(mapping, {
				[id]: name
			});
		}
...
export async function getOriginSourceId(
	...
	let originSourceId: any = null;

	try {
		const [idObj] = await transacting
			.select('id')
			.from('bookbrainz.origin_source')
			.where('name', '=', source);
	...
	if (!originSourceId) {
		try {
			[originSourceId] = await transacting
				.insert([
					{
						name: source
					}
...
export async function getOriginSourceFromId(
	...
	// Should error loudly if anything goes wrong
	const [nameObj] = await transacting
		.select('name')
		.from('bookbrainz.origin_source')
...
export async function getImportDetails(
	...
	// Should error loudly if anything goes wrong
	const [details] = await transacting
		.select('*')
