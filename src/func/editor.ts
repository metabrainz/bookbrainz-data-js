...
export function deleteEditorByMetaBrainzID(knex) {
	return (metabrainzUserID) =>
		knex.transaction((trx) => {
			...
			const editorIDPromise = getEditorIDByMetaBrainzID(
				trx,
				metabrainzUserID
			);
			return editorIDPromise.then((editorID) => {
				...
				const clearEditorPromise = clearEditorByID(trx, editorID);
				// ... and languages
				const clearEditorLanguagesPromise = clearEditorLanguagesByEditorID(
					trx,
					editorID
				);
