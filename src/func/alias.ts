...
export async function updateAliasSet(
	...
	const addedItems = getAddedItems(oldSetItems, newSetItems, comparisonFunc);
	...
	const isSetUnmodified =
		newDefaultAlias.id === oldDefaultAliasId &&
	...
	addedItems.forEach((alias, index, itemsArray) => {
		...
		const existingAliasSameLanguage =
			unchangedItems.findIndex(
		...
		itemsArray
			...
			.filter(
				({languageId, primary}) =>
					primary && languageId === alias.languageId
			)
			.forEach((otherAlias) => {
				otherAlias.primary = false;
			});
	});
	...
	const newSetItemCollection = await newSet.related('aliases').fetch({
		transacting
	});
...
export async function getAliasByIds(
	...
	const aliases = await transacting
		.select('*')
