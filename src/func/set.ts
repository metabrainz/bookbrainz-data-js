 ...
 */
export function getComparisonFunc(
 ...
 * arrays
 */
export function getUnchangedItems<Item>(
	oldSet: Array<Item>,
	newSet: Array<Item>,
	comparisonFunc: (obj: Item, other: Item) => boolean
): Array<Item> {
	return _.uniqWith(
		_.intersectionWith(oldSet, newSet, comparisonFunc),
		comparisonFunc
	);
}
 ...
 * arrays
 */
export function getAddedItems<Item>(
	oldSet: Array<Item>,
	newSet: Array<Item>,
	comparisonFunc: (obj: Item, other: Item) => boolean
): Array<Item> {
	return _.uniqWith(
		_.differenceWith(newSet, oldSet, comparisonFunc),
		comparisonFunc
	);
}
 ...
 * arrays
 */
export function getRemovedItems<Item>(
	oldSet: Array<Item>,
	newSet: Array<Item>,
	comparisonFunc: (obj: Item, other: Item) => boolean
): Array<Item> {
	return _.uniqWith(
		_.differenceWith(oldSet, newSet, comparisonFunc),
		comparisonFunc
	);
}
export const removeItemsFromSet = getRemovedItems;
export async function createNewSetWithItems<Item>(
	orm: any,
	transacting: Transaction,
	SetModel: any,
	unchangedItems: Array<Item>,
	...
	const newSetItemsCollectionAttached = await newSetItemsCollection.attach(
		_.map(unchangedItems, idAttribute),
		{
			transacting
		}
	);
	await Promise.all(
		_.map(addedItems, (ident: any) =>
			newSetItemsCollectionAttached.create(_.omit(ident, idAttribute), {
				transacting
			})
		)
	);
